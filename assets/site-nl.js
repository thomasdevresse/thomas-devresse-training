(() => {
  ...
  function closeMenu(restoreFocus = false) {
    ...
    menuButton.setAttribute('aria-label', 'Navigatie openen');
  ...
  if (menuButton && menu) {
    menuButton.addEventListener('click', () => {
      ...
      menuButton.setAttribute('aria-label', willOpen ? 'Navigatie sluiten' : 'Navigatie openen');
  ...
  function validateStep(index) {
    ...
    if (firstInvalid) {
      ...
      errorBox.textContent = 'Vul de verplichte velden in. Geschreven antwoorden mogen niet leeg zijn of alleen spaties bevatten.';
      firstInvalid.focus();
      return false;
    }
    errorBox.textContent = '';
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (value('website')) {
      form.reset();
      statusBox.textContent = 'Bedankt. Je aanvraag is goed ontvangen.';
      return;
    }
    ...
    const message = [
      `Doel: ${answers.goal}`,
      `Betekenisvolle vooruitgang: ${answers.goalDetail}`,
      `Ervaring: ${answers.experience}`,
      `Huidige frequentie: ${answers.frequency}`,
      `Gewenste hulp bij training: ${answers.trainingHelp || 'Niet opgegeven'}`,
      `Locatie: ${answers.location}`,
      `Beschikbaarheid: ${answers.availability}`
    ].join('\n\n');
    ...
    const payload = {
      ...
      email: value('email'),
      phone: value('phone'),
      subject: `Coachingaanvraag — ${value('goal')}`,
      answers,
      language: 'nl',
